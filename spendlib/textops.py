...
def head_tail(text: str, head: int, tail: int, label: str) -> str | None:
    ...
    parts = lines[:head]
    parts.append(f"[spend] … {omitted} {label} omitted …")
...
def drain(text: str, max_groups: int = 40, samples: int = 1) -> str | None:
    ...
    out = [f"[spend] {len(text.splitlines())} lines → {len(groups)} templates"]
    for i, (tmpl, members) in enumerate(rows):
        if i >= max_groups:
            out.append(f"[spend] +{len(rows) - max_groups} more templates")
            break
