...
If the CLI fails, Read `~/.scrim/last.txt`.
...
- If Tools shows 0 calls, the hook is not installed. Tell them to restart with `--plugin-dir` or `/plugin install scrim@scrim`.
